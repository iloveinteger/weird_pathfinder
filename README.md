...
기본 실행은 실제 API를 호출하지 않는 mock mode입니다. 별도 serverless backend를 배포하고 real mode를 선택하면 Kakao/TAGO/서울 API adapter와 Kakao Maps를 사용할 수 있습니다. 서울 도심의 작은 mock 교통망은 Normal/Hard 회귀 테스트용으로 계속 유지합니다. KTX, SRT, 고속·시외버스와 항공은 범위에 포함하지 않습니다.
